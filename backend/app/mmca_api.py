...
# 라운드마다 8줄이 쌓인다 — */2 그리드에서 하루 1,900줄이고, 그만큼 남은 미지의
...
#
# "방 수" 는 라운드마다 같지 않다 — 상시 0002 인 방은 30분에 한 번만 끼므로
# (collector 의 _PROBE_MINUTES) 보통 라운드는 10방, probe 라운드는 17방이다.
# 산식을 묶는 쪽은 큰 값, 즉 probe 라운드다.
#
...
# 상류가 느려져도 견딘다.
#
# 3.0 이 실제로 버티는지는 2026-09-08(화, 세 관 모두 개관)에 확인했다. 10:00
# ~18:00 에 241라운드 × 17방 = 4,097콜이 전부 성공했고, 스킵된 라운드도 방을
# 잃은 라운드도 없었다. 같은 격자의 09-06 은 15:45~18:00 에 라운드가 4~38분씩
# 밀렸지만 그 구간 라운드가 2~5방까지 떨어진 것이라 예산 초과가 아니라 상류
# 장애다 — 그런 시간대에는 라운드를 지켜도 가져올 데이터가 없다.
#
