...
describe("DateTabs", () => {
  it("labels the first date as today and the rest by weekday and date", () => {
    ...
    // 요일 위 / 날짜 아래 두 줄이라 접근성 이름은 두 줄이 이어진 형태다.
    // 2026-08-24 는 월요일
    expect(screen.getByRole("tab", { name: "오늘 8/23" })).toBeInTheDocument();
  ...
  it("marks only the selected tab as selected", () => {
    ...
    expect(screen.getByRole("tab", { name: "오늘 8/23" })).toHaveAttribute("aria-selected", "false");
  });
