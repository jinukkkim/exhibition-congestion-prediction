...
export interface VenueInfo {
  ...
  //
  // 이 문자열은 관 정보 표에 그대로 뜨는 사람이 읽는 요약이고, 기계가 읽는
  // 전체 목록은 shared/museum-holidays.json 에 있다 — 둘은 서로 어긋나지
  // 않게 맞춰야 한다. 서울관은 이 문자열이 "1월 1일, 설날, 추석" 이지만
  // JSON 은 2026-09-08 과 2026-12-01(임시 휴관)도 닫는다고 적는다; 그 두
  // 날짜는 일회성 임시 휴관이라 여기 다 나열하면 표가 읽기 어려워져
  // 일부러 뺐다.
  closedDays: string;
  homepage: string;
}
